...
public class GoogleService {
    ...
    public String translateVI(String input, Language language) {

        String data = "translate,sl:%s,tl:vi,st:%s,id:%d,qc:true,ac:true,_id:tw-async-translate,_pms:s,_fmt:pc";

        try {
            Document document = Jsoup.connect("https://www.google.com.vn/async/translate?yv=3")
                    .userAgent(UserAgent.getUserAgent())
                    .timeout(30000)
                    .data("async", String.format(data, language.getName(), URLEncoder.encode(input, "UTF-8"), new Date().getTime()))
                    .post();
